#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <errno.h>
#include <sys/socket.h>
#include <sys/un.h>
#include <arpa/inet.h>
#include <netinet/in.h>
#include <fcntl.h>
#include <sys/mman.h>
#include <sys/stat.h>
#include <getopt.h>

#define BUF_SIZE 1024
#define MAX_ATOMS 1000000000000000000ULL

typedef struct {
    unsigned long long carbon;
    unsigned long long oxygen;
    unsigned long long hydrogen;
} inventory_t;

inventory_t *inv = NULL;
int inv_fd = -1;
size_t inv_size = sizeof(inventory_t);
int using_save_file = 0;

void lock_inventory(int fd, int write) {
    if (fd < 0) return;
    struct flock fl = {0};
    fl.l_type = write ? F_WRLCK : F_RDLCK;
    fl.l_whence = SEEK_SET;
    fl.l_start = 0;
    fl.l_len = inv_size;
    fcntl(fd, F_SETLKW, &fl);
}
void unlock_inventory(int fd) {
    if (fd < 0) return;
    struct flock fl = {0};
    fl.l_type = F_UNLCK;
    fl.l_whence = SEEK_SET;
    fl.l_start = 0;
    fl.l_len = inv_size;
    fcntl(fd, F_SETLKW, &fl);
}
void sync_inventory_for_read() {
    if (inv && using_save_file) {
        msync(inv, inv_size, MS_INVALIDATE);
    }
}
void print_state() {
    sync_inventory_for_read();
    if (inv) {
        printf("Warehouse State: CARBON=%llu OXYGEN=%llu HYDROGEN=%llu\n",
            inv->carbon, inv->oxygen, inv->hydrogen);
        fflush(stdout);
    }
}
void save_inventory() {
    if (using_save_file && inv_fd >= 0 && inv) {
        msync(inv, inv_size, MS_SYNC);
    }
}
void load_inventory_if_needed(const char *save_file, unsigned long long carbon, unsigned long long oxygen, unsigned long long hydrogen) {
    if (!save_file) return;
    using_save_file = 1;
    int fd = open(save_file, O_RDWR | O_CREAT, 0666);
    if (fd < 0) { perror("open save file"); exit(1); }
    struct stat st;
    if (fstat(fd, &st) < 0) { perror("fstat"); exit(1); }
    if (st.st_size < 0 || (size_t)st.st_size < inv_size) {
        // קובץ חדש – אתחל ערכים
        if (ftruncate(fd, inv_size) < 0) { perror("ftruncate"); exit(1); }
        inventory_t zero = {carbon, oxygen, hydrogen};
        ssize_t wbytes = write(fd, &zero, inv_size);
        if (wbytes != (ssize_t)inv_size) {
            perror("write init"); exit(1);
        }
    }
    inv = (inventory_t*)mmap(NULL, inv_size, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
    if (inv == MAP_FAILED) { perror("mmap"); exit(1); }
    inv_fd = fd;
}
void close_inventory() {
    if (inv && using_save_file) {
        msync(inv, inv_size, MS_SYNC);
        munmap(inv, inv_size);
        close(inv_fd);
    }
}
void ensure_inventory(unsigned long long *carbon, unsigned long long *oxygen, unsigned long long *hydrogen) {
    sync_inventory_for_read();
    if (inv) {
        *carbon = inv->carbon;
        *oxygen = inv->oxygen;
        *hydrogen = inv->hydrogen;
    }
}
void update_inventory(unsigned long long carbon, unsigned long long oxygen, unsigned long long hydrogen) {
    if (inv) {
        inv->carbon = carbon;
        inv->oxygen = oxygen;
        inv->hydrogen = hydrogen;
        save_inventory();
    }
}

int handle_tcp_command(const char *buf) {
    unsigned long long num, c, o, h;
    ensure_inventory(&c, &o, &h);
    if (sscanf(buf, "ADD CARBON %llu", &num) == 1) {
        if (c + num > MAX_ATOMS) num = MAX_ATOMS - c;
        c += num;
        update_inventory(c, o, h);
        print_state();
        return 0;
    } else if (sscanf(buf, "ADD OXYGEN %llu", &num) == 1) {
        if (o + num > MAX_ATOMS) num = MAX_ATOMS - o;
        o += num;
        update_inventory(c, o, h);
        print_state();
        return 0;
    } else if (sscanf(buf, "ADD HYDROGEN %llu", &num) == 1) {
        if (h + num > MAX_ATOMS) num = MAX_ATOMS - h;
        h += num;
        update_inventory(c, o, h);
        print_state();
        return 0;
    } else if (strncmp(buf, "EXIT", 4) == 0) {
        printf("Client requested to close its connection.\n");
        fflush(stdout);
        return 1;
    } else {
        fprintf(stderr, "Invalid command: %s", buf);
        fflush(stderr);
        return -1;
    }
}
int handle_udp_command(const char *buf, char *msg) {
    unsigned long long num, c, o, h;
    ensure_inventory(&c, &o, &h);
    if (sscanf(buf, "DELIVER WATER %llu", &num) == 1) {
        if (h >= 2*num && o >= num) {
            h -= 2*num;
            o -= num;
            update_inventory(c, o, h);
            snprintf(msg, BUF_SIZE, "SUPPLIED WATER %llu\n", num);
            print_state();
            return 1;
        } else {
            snprintf(msg, BUF_SIZE, "FAILED WATER %llu\n", num);
            return 0;
        }
    } else if (sscanf(buf, "DELIVER CARBON DIOXIDE %llu", &num) == 1) {
        if (c >= num && o >= 2*num) {
            c -= num;
            o -= 2*num;
            update_inventory(c, o, h);
            snprintf(msg, BUF_SIZE, "SUPPLIED CARBON DIOXIDE %llu\n", num);
            print_state();
            return 1;
        } else {
            snprintf(msg, BUF_SIZE, "FAILED CARBON DIOXIDE %llu\n", num);
            return 0;
        }
    } else if (sscanf(buf, "DELIVER ALCOHOL %llu", &num) == 1) {
        if (c >= 2*num && h >= 6*num && o >= num) {
            c -= 2*num;
            h -= 6*num;
            o -= num;
            update_inventory(c, o, h);
            snprintf(msg, BUF_SIZE, "SUPPLIED ALCOHOL %llu\n", num);
            print_state();
            return 1;
        } else {
            snprintf(msg, BUF_SIZE, "FAILED ALCOHOL %llu\n", num);
            return 0;
        }
    } else if (sscanf(buf, "DELIVER GLUCOSE %llu", &num) == 1) {
        if (c >= 6*num && h >= 12*num && o >= 6*num) {
            c -= 6*num;
            h -= 12*num;
            o -= 6*num;
            update_inventory(c, o, h);
            snprintf(msg, BUF_SIZE, "SUPPLIED GLUCOSE %llu\n", num);
            print_state();
            return 1;
        } else {
            snprintf(msg, BUF_SIZE, "FAILED GLUCOSE %llu\n", num);
            return 0;
        }
    } else {
        fprintf(stderr, "Invalid UDP command: %s", buf);
        snprintf(msg, BUF_SIZE, "ERROR INVALID COMMAND\n");
        return -1;
    }
}

void compute_molecule_counts(unsigned long long c, unsigned long long o, unsigned long long h,
    unsigned long long *w, unsigned long long *cd, unsigned long long *alc, unsigned long long *glu) {
    *w = (h/2 < o) ? h/2 : o;
    *cd = (c < o/2) ? c : o/2;
    *alc = c/2;
    if (h/6 < *alc) *alc = h/6;
    if (o < *alc) *alc = o;
    *glu = c/6;
    if (h/12 < *glu) *glu = h/12;
    if (o/6 < *glu) *glu = o/6;
}
void handle_bar_command(const char *buf) {
    unsigned long long c, o, h, w, cd, alc, glu;
    ensure_inventory(&c, &o, &h);
    compute_molecule_counts(c, o, h, &w, &cd, &alc, &glu);
    if (strncmp(buf, "GEN SOFT DRINK", 14) == 0) {
        unsigned long long min = w;
        if (cd < min) min = cd;
        if (glu < min) min = glu;
        printf("SOFT DRINKS AVAILABLE: %llu\n", min);
    } else if (strncmp(buf, "GEN VODKA", 9) == 0) {
        unsigned long long min = w;
        if (alc < min) min = alc;
        if (glu < min) min = glu;
        printf("VODKA DRINKS AVAILABLE: %llu\n", min);
    } else if (strncmp(buf, "GEN CHAMPAGNE", 13) == 0) {
        unsigned long long min = w;
        if (cd < min) min = cd;
        if (alc < min) min = alc;
        printf("CHAMPAGNE DRINKS AVAILABLE: %llu\n", min);
    } else {
        fprintf(stderr, "Invalid BAR command: %s", buf);
    }
    fflush(stdout);
}

void print_usage(const char *prog) {
    fprintf(stderr,
        "Usage: %s -T <tcp-port> -U <udp-port> [options...]\n"
        "  -T, --tcp-port <port>       TCP port (required)\n"
        "  -U, --udp-port <port>       UDP port (required)\n"
        "  -o, --oxygen <count>        Initial OXYGEN atoms (optional, default 0)\n"
        "  -c, --carbon <count>        Initial CARBON atoms (optional, default 0)\n"
        "  -h, --hydrogen <count>      Initial HYDROGEN atoms (optional, default 0)\n"
        "  -t, --timeout <seconds>     Timeout in seconds (optional, default: none)\n"
        "  -s, --stream-path <path>    UDS stream socket file (optional)\n"
        "  -d, --datagram-path <path>  UDS datagram socket file (optional)\n"
        "  -f, --save-file <path>      Save/load inventory from file (optional)\n"
        , prog);
}

int main(int argc, char *argv[]) {
    int tcp_port = -1, udp_port = -1;
    unsigned long long initial_oxygen = 0, initial_carbon = 0, initial_hydrogen = 0;
    int timeout_seconds = -1;
    char *stream_path = NULL, *datagram_path = NULL, *save_file = NULL;
    int uds_stream_fd = -1, uds_dgram_fd = -1;

    static struct option long_opts[] = {
        {"tcp-port",   required_argument, 0, 'T'},
        {"udp-port",   required_argument, 0, 'U'},
        {"oxygen",     required_argument, 0, 'o'},
        {"carbon",     required_argument, 0, 'c'},
        {"hydrogen",   required_argument, 0, 'h'},
        {"timeout",    required_argument, 0, 't'},
        {"stream-path", required_argument, 0, 's'},
        {"datagram-path", required_argument, 0, 'd'},
        {"save-file", required_argument, 0, 'f'},
        {0,0,0,0}
    };

    int opt;
    while ((opt = getopt_long(argc, argv, "T:U:o:c:h:t:s:d:f:", long_opts, NULL)) != -1) {
        switch(opt) {
            case 'T':
                tcp_port = atoi(optarg); break;
            case 'U':
                udp_port = atoi(optarg); break;
            case 'o':
                initial_oxygen = strtoull(optarg, NULL, 10); break;
            case 'c':
                initial_carbon = strtoull(optarg, NULL, 10); break;
            case 'h':
                initial_hydrogen = strtoull(optarg, NULL, 10); break;
            case 't':
                timeout_seconds = atoi(optarg); break;
            case 's':
                stream_path = strdup(optarg); break;
            case 'd':
                datagram_path = strdup(optarg); break;
            case 'f':
                save_file = strdup(optarg); break;
            default:
                print_usage(argv[0]);
                exit(1);
        }
    }

    if (tcp_port <= 0 || udp_port <= 0) {
        fprintf(stderr, "Error: Both TCP and UDP ports are required!\n");
        print_usage(argv[0]);
        exit(1);
    }

    // inventory setup
    if (save_file) {
        load_inventory_if_needed(save_file, initial_carbon, initial_oxygen, initial_hydrogen);
    }

    // אם לא מצביע על mmap, נשתמש במשתנים רגילים:
    unsigned long long carbon = initial_carbon, oxygen = initial_oxygen, hydrogen = initial_hydrogen;
    if (!inv) {
        inv = (inventory_t*)malloc(sizeof(inventory_t));
        inv->carbon = carbon;
        inv->oxygen = oxygen;
        inv->hydrogen = hydrogen;
    }

    // TCP setup
    int listen_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (listen_fd < 0) { perror("socket"); exit(1); }
    struct sockaddr_in addr;
    memset(&addr, 0, sizeof(addr));
    addr.sin_family = AF_INET;
    addr.sin_addr.s_addr = INADDR_ANY;
    addr.sin_port = htons(tcp_port);
    if (bind(listen_fd, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
        perror("bind TCP"); close(listen_fd); exit(1);
    }
    if (listen(listen_fd, 10) < 0) {
        perror("listen"); close(listen_fd); exit(1);
    }

    // UDP setup
    int udp_sock = socket(AF_INET, SOCK_DGRAM, 0);
    if (udp_sock < 0) { perror("socket UDP"); exit(1); }
    struct sockaddr_in udp_addr;
    memset(&udp_addr, 0, sizeof(udp_addr));
    udp_addr.sin_family = AF_INET;
    udp_addr.sin_addr.s_addr = INADDR_ANY;
    udp_addr.sin_port = htons(udp_port);
    if (bind(udp_sock, (struct sockaddr*)&udp_addr, sizeof(udp_addr)) < 0) {
        perror("bind UDP"); close(udp_sock); exit(1);
    }

    // UDS Stream setup (optional)
    if (stream_path) {
        uds_stream_fd = socket(AF_UNIX, SOCK_STREAM, 0);
        if (uds_stream_fd < 0) { perror("socket UDS STREAM"); exit(1); }
        struct sockaddr_un addr_un;
        memset(&addr_un, 0, sizeof(addr_un));
        addr_un.sun_family = AF_UNIX;
        strncpy(addr_un.sun_path, stream_path, sizeof(addr_un.sun_path)-1);
        unlink(stream_path);
        if (bind(uds_stream_fd, (struct sockaddr*)&addr_un, sizeof(addr_un)) < 0) {
            perror("bind UDS STREAM"); close(uds_stream_fd); exit(1);
        }
        if (listen(uds_stream_fd, 10) < 0) {
            perror("listen UDS STREAM"); close(uds_stream_fd); exit(1);
        }
    }

    // UDS Datagram setup (optional)
    if (datagram_path) {
        uds_dgram_fd = socket(AF_UNIX, SOCK_DGRAM, 0);
        if (uds_dgram_fd < 0) { perror("socket UDS DGRAM"); exit(1); }
        struct sockaddr_un addr_un;
        memset(&addr_un, 0, sizeof(addr_un));
        addr_un.sun_family = AF_UNIX;
        strncpy(addr_un.sun_path, datagram_path, sizeof(addr_un.sun_path)-1);
        unlink(datagram_path);
        if (bind(uds_dgram_fd, (struct sockaddr*)&addr_un, sizeof(addr_un)) < 0) {
            perror("bind UDS DGRAM"); close(uds_dgram_fd); exit(1);
        }
    }

    // STDIN non-blocking
    int flags = fcntl(STDIN_FILENO, F_GETFL, 0);
    fcntl(STDIN_FILENO, F_SETFL, flags | O_NONBLOCK);

    fd_set master;
    int fdmax = listen_fd;
    if (udp_sock > fdmax) fdmax = udp_sock;
    if (STDIN_FILENO > fdmax) fdmax = STDIN_FILENO;
    if (uds_stream_fd > fdmax) fdmax = uds_stream_fd;
    if (uds_dgram_fd > fdmax) fdmax = uds_dgram_fd;

    FD_ZERO(&master);
    FD_SET(listen_fd, &master);
    FD_SET(udp_sock, &master);
    FD_SET(STDIN_FILENO, &master);
    if (uds_stream_fd != -1) FD_SET(uds_stream_fd, &master);
    if (uds_dgram_fd != -1) FD_SET(uds_dgram_fd, &master);

    printf("Bar Drinks server: TCP port %d, UDP port %d\n", tcp_port, udp_port);
    printf("Initial state: CARBON=%llu OXYGEN=%llu HYDROGEN=%llu\n", inv->carbon, inv->oxygen, inv->hydrogen);
    if (stream_path) printf("UDS Stream socket: %s\n", stream_path);
    if (datagram_path) printf("UDS Datagram socket: %s\n", datagram_path);
    if (save_file) printf("Inventory save file: %s\n", save_file);
    if (timeout_seconds > 0)
        printf("Timeout: %d seconds of inactivity\n", timeout_seconds);
    printf("To close all connections type SHUTDOWN.\n");

    int running = 1;
    while (running) {
        fd_set read_fds = master;
        struct timeval tv, *tvptr = NULL;
        if (timeout_seconds > 0) {
            tv.tv_sec = timeout_seconds;
            tv.tv_usec = 0;
            tvptr = &tv;
        }
        int sel = select(fdmax+1, &read_fds, NULL, NULL, tvptr);
        if (sel == -1) {
            perror("select"); exit(1);
        } else if (sel == 0) {
            printf("Timeout (%d seconds) reached, no activity. Exiting.\n", timeout_seconds);
            break;
        }

        for (int i = 0; i <= fdmax; ++i) {
            if (FD_ISSET(i, &read_fds)) {
                if (i == listen_fd) {
                    struct sockaddr_in client_addr;
                    socklen_t addrlen = sizeof(client_addr);
                    int newfd = accept(listen_fd, (struct sockaddr*)&client_addr, &addrlen);
                    if (newfd == -1) { perror("accept"); continue; }
                    FD_SET(newfd, &master);
                    if (newfd > fdmax) fdmax = newfd;
                } else if (i == udp_sock) {
                    char buf[BUF_SIZE], reply[BUF_SIZE];
                    struct sockaddr_in cliaddr;
                    socklen_t len = sizeof(cliaddr);
                    ssize_t n = recvfrom(udp_sock, buf, sizeof(buf)-1, 0, (struct sockaddr*)&cliaddr, &len);
                    if (n < 0) continue;
                    buf[n] = 0;
                    lock_inventory(inv_fd, 1);
                    handle_udp_command(buf, reply);
                    unlock_inventory(inv_fd);
                    sendto(udp_sock, reply, strlen(reply), 0, (struct sockaddr*)&cliaddr, len);
                } else if (i == uds_stream_fd) {
                    int newfd = accept(uds_stream_fd, NULL, NULL);
                    if (newfd == -1) { perror("accept UDS STREAM"); continue; }
                    FD_SET(newfd, &master);
                    if (newfd > fdmax) fdmax = newfd;
                } else if (i == uds_dgram_fd) {
                    char buf[BUF_SIZE], reply[BUF_SIZE];
                    struct sockaddr_un cliaddr;
                    socklen_t len = sizeof(cliaddr);
                    ssize_t n = recvfrom(uds_dgram_fd, buf, sizeof(buf)-1, 0, (struct sockaddr*)&cliaddr, &len);
                    if (n < 0) continue;
                    buf[n] = 0;
                    lock_inventory(inv_fd, 1);
                    handle_udp_command(buf, reply);
                    unlock_inventory(inv_fd);
                    sendto(uds_dgram_fd, reply, strlen(reply), 0, (struct sockaddr*)&cliaddr, len);
                } else if (i == STDIN_FILENO) {
                    char input_buf[BUF_SIZE];
                    if (fgets(input_buf, sizeof(input_buf), stdin)) {
                        if (strncmp(input_buf, "SHUTDOWN", 8) == 0) {
                            printf("Server shutting down by terminal command.\n");
                            for (int j = 0; j <= fdmax; ++j) {
                                if (FD_ISSET(j, &master) && j != listen_fd && j != udp_sock
                                    && j != STDIN_FILENO && j != uds_stream_fd && j != uds_dgram_fd) {
                                    close(j);
                                    FD_CLR(j, &master);
                                }
                            }
                            close(listen_fd);
                            close(udp_sock);
                            if (uds_stream_fd != -1) close(uds_stream_fd);
                            if (uds_dgram_fd != -1) close(uds_dgram_fd);
                            if (stream_path) unlink(stream_path);
                            if (datagram_path) unlink(datagram_path);
                            close_inventory();
                            running = 0;
                            break;
                        } else if (strncmp(input_buf, "GEN", 3) == 0) {
                            handle_bar_command(input_buf);
                        }
                    }
                } else {
                    // TCP/UDS_STREAM חיבור קיים
                    char buf[BUF_SIZE];
                    ssize_t nbytes = recv(i, buf, sizeof(buf)-1, 0);
                    if (nbytes <= 0) {
                        close(i);
                        FD_CLR(i, &master);
                    } else {
                        buf[nbytes] = '\0';
                        lock_inventory(inv_fd, 1);
                        int res = handle_tcp_command(buf);
                        unlock_inventory(inv_fd);
                        if (res == 1) {
                            close(i);
                            FD_CLR(i, &master);
                        }
                    }
                }
            }
        }
    }
    if (stream_path) unlink(stream_path);
    if (datagram_path) unlink(datagram_path);
    close_inventory();
    printf("Server exited.\n");
    return 0;
}